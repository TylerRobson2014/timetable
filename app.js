...
function getRandomColor() {
    ...
    var colorMemory = [];
    socket.on('fillCell', function (msg) {
        console.log('Message Received: ', msg);
	console.log(socket.id);
	UserID = socket.id;
	for (var i = 0; i< users.length;i++){
	...
	msg[2] = getRandomColor();
	//while (colorMemory.indexOf(msg[2]) > -1){
	//	msg[2] = getRandomColor();
	//}
	//colorMemory.push(msg[2]);
	//console.log("emit",msg.X,msg.Y,msg.col);
        socket.broadcast.emit('draw', msg);
    });

    socket.on('clip', function () {
        socket.broadcast.emit('clipc', "");
    });
	...
	UserID = socket.id;
	console.log("server discoonect msg");
	for (var i = 0; i< users.length;i++){
		if (users[i].uID == socket.id){
			var displayName = users[i].uName;
			if (i > -1){
